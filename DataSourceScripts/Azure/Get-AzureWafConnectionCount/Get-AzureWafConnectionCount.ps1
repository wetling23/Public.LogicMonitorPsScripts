<#
    .DESCRIPTION
        Connect to Azure AD, and query Web Applicaiton Firewall log.
    .NOTES
...
$logFile = "$logDirPath\datasource-Get-AzureWafConnectionCount-collection-$computer.log"
...
#region Main
$message = ("{0}: Querying Azure Operational Insights for connection counts." -f ([datetime]::Now).ToString("yyyy-MM-dd`THH:mm:ss"))
$message | Out-File -FilePath $logFile -Append

$query = 'AzureDiagnostics | where ResourceProvider == "MICROSOFT.NETWORK" and Category == "ApplicationGatewayFirewallLog" | summarize count() by action_s'
...
If ($results.Results.'action_s') {
    $message = ("{0}: Returning data to LogicMonitor:`r`n{1}" -f ([datetime]::Now).ToString("yyyy-MM-dd`THH:mm:ss"), ($results.Results | Out-String))
    $message | Out-File -FilePath $logFile -Append

    Write-Host ("AllowedConnections={0}" -f (($results.Results | Where-Object { $_.'action_s' -eq "Allowed" }) | Select-Object -ExpandProperty count_))
    Write-Host ("MatchedConnections={0}" -f (($results.Results | Where-Object { $_.'action_s' -eq "Matched" }) | Select-Object -ExpandProperty count_))
    Write-Host ("BlockedConnections={0}" -f (($results.Results | Where-Object { $_.'action_s' -eq "Blocked" }) | Select-Object -ExpandProperty count_))
}
